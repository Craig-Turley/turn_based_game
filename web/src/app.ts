...
class StateMainMenu implements StateDriver {
  game: Game
...
type TileSet = { [key: number]: Tile };


function newTileSet(tiles: { key: number, path: string, size: Vector }[]): TileSet {
  const tileMap: TileSet = {};
  tiles.forEach((tile) => {
    const img = new Image();
    img.src = tile.path;
    tileMap[tile.key] = { tile: img, size: tile.size }
  });

  return tileMap
}

function newBackDrop(paths: string[]): BackDrop {
  const backDrop: BackDrop = { layers: [], }

  paths.forEach((path) => {
    const img = new Image();
    img.src = path;
    backDrop.layers.push(img);
  });

  return backDrop;
}

const JUNGLE_BACKGROUND = [
  './assets/jungle_asset_pack/parallax_background/plx-1.png',
  './assets/jungle_asset_pack/parallax_background/plx-2.png',
  './assets/jungle_asset_pack/parallax_background/plx-3.png',
  './assets/jungle_asset_pack/parallax_background/plx-4.png',
...
const JUNGLE_TILES = [
  {
    key: 1,
    path: "./assets/platforms/tiles/tile_0003.png",
    size: new Vector(21, 21),
  },
  {
    key: 2,
    path: "./assets/platforms/tiles/tile_0032.png",
    size: new Vector(21, 21),
  }
]
...
class DisplayDriver {
  ...
  ctxWidth: number = BASE_WIDTH;
  ctxHeight: number = BASE_HEIGHT;
  xOffset: number = 0;
  yOffset: number = 0;
  scale: number = 1;
  backDrop: BackDrop;
  ...
  constructor(ctx: CanvasRenderingContext2D) {
    this.ctx = ctx;
    this.backDrop = newBackDrop(JUNGLE_BACKGROUND);
    this.tileSet = newTileSet(JUNGLE_TILES);
  ...
  resize(): void {
    const boundingBox = canvas.parentElement!.getBoundingClientRect();
    const width = Math.max(boundingBox.width, this.baseWidth);
    const height = Math.max(boundingBox.height, this.baseHeight);

    const scale = Math.floor(Math.min(width / this.baseWidth, height / this.baseHeight));

    this.ctxWidth = scale * this.baseWidth;
    this.ctxHeight = scale * this.baseHeight;
    this.scale = scale;

    this.ctx.canvas.width = width;
    this.ctx.canvas.height = height;

    this.xOffset = Math.floor((width - this.ctxWidth) / 2);
    this.yOffset = Math.floor((height - this.ctxHeight) / 2);
  }
  ...
  drawBackDrop() {
    const overflow = Math.floor(this.ctx.canvas.width / this.ctxWidth);
    this.backDrop.layers.forEach((layer) => {
      // accounts and draws overflow
      for (let x = 0 - overflow * this.ctxWidth; x <= overflow * this.ctxWidth; x += this.ctxWidth) {
        this.ctx.drawImage(layer, this.cX(x), 0, this.ctxWidth, this.ctxHeight);
      }
    });
  }
  ...
  drawStage() {
    const floorTile = this.tileSet[1];
    const floorTiley = this.ctxHeight - floorTile.size.y * this.scale;
    this.drawRow(floorTile, floorTiley);

    const undergroundTile = this.tileSet[2];
    for (let y = this.ctxHeight; y <= this.ctx.canvas.height; y += undergroundTile.size.y * this.scale) {
      this.drawRow(undergroundTile, y);
    }


  };

  drawRow(tile: Tile, y: number) {
    const overflowTiles = Math.ceil(((this.ctx.canvas.width - this.ctxWidth) / 2) / (tile.size.x * this.scale));
    const overflowTilesWidth = overflowTiles * tile.size.x * this.scale;
    for (let x = 0 - overflowTilesWidth; x < this.ctxWidth + overflowTilesWidth; x += tile.size.x * this.scale) {
      this.ctx.drawImage(tile.tile, this.cX(x), y, tile.size.x * this.scale, tile.size.y * this.scale);
    }
  }
...
class Game {
  ...
  draw(time: number) {
    this.displayDriver.draw(time);
    requestAnimationFrame((t) => {
      this.draw(t);
    });
  }

  setState(gameState: StateDriver) {
